...
const GREETING =
  "Hey — I'm Drew, or rather, an agent I built end-to-end to speak for me (Cloudflare Worker + an open-source LLM via Workers AI, with guardrails so I can't invent numbers). Ask me what I actually shipped at Brivo, how I use LLMs in QA, or how to hire me. I'll keep it concrete."
...
// Parse Cloudflare Workers AI SSE stream — chunks look like
// `data: {"response":"chunk text"}`, terminated by `data: [DONE]`.
async function* streamAgent(response) {
...
const AgentDock = () => {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      let acc = ''
      for await (const chunk of streamAgent(res)) {
        acc += chunk
        setMessages((curr) => {
        ...
        {open && (
          ...
          >
            ...
            <div className="agent-disclaimer">
              answers generated by an open-source llm (cloudflare workers ai) · may occasionally err — confirm key details with drew directly
            </div>
          </motion.div>
        )}
      </AnimatePresence>
